...
const AdminAddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (jsonResponse.success) {
        setMessage("✅ Product successfully uploaded to Cloudinary & Database!");
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md border border-gray-200">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Admin Panel: Add New Product</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Feedback Message */}
        {message && (
          <div className={`p-3 mt-4 rounded font-medium text-sm ${message.includes('✅') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {message}
          </div>
        )}
      </form>
